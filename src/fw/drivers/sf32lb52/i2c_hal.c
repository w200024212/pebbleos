...
void i2c_irq_handler(I2CBus *bus) {
  I2C_HandleTypeDef *handle = &(bus->hal->i2c_state->hi2c);
...
static HAL_StatusTypeDef i2c_hal_master_xfer(struct I2CBusHal *i2c_hal, struct rt_i2c_msg msgs[],
                                             uint32_t num) {
  uint32_t index = 0;
  struct I2CBusHal *hal = NULL;
  ...
  hal = i2c_hal;
  __HAL_I2C_ENABLE(&(hal->i2c_state->hi2c));
  for (index = 0; index < num; index++) {
    ...
    if (msg->flags & RT_I2C_MEM_ACCESS) {
      ...
      PBL_LOG(LOG_LEVEL_DEBUG,
              "msg->addr = 0x%x, msg->mem_addr = 0x%x, type = %d, buf = 0x%x, len = 0x%d, timeout "
              "= %d;",
              (unsigned int)msg->addr, (unsigned int)msg->mem_addr, (int)mem_addr_type,
              (unsigned int)msg->buf, (int)msg->len, (int)hal->i2c_state->i2c_conf.timeout);
#endif
      if (msg->flags & RT_I2C_RD) {
        if (hal->i2c_state->hdma.Instance) {
          HAL_DMA_Init(hal->i2c_state->hi2c.hdmarx);
          mpu_dcache_invalidate(msg->buf, msg->len);
          status = HAL_I2C_Mem_Read_DMA(&hal->i2c_state->hi2c, msg->addr, msg->mem_addr,
                                        mem_addr_type, msg->buf, msg->len);
        } else if (hal->i2c_state->int_enabled) {
          status = HAL_I2C_Mem_Read_IT(&hal->i2c_state->hi2c, msg->addr, msg->mem_addr,
                                       mem_addr_type, msg->buf, msg->len);
        } else {
          status = HAL_I2C_Mem_Read(&hal->i2c_state->hi2c, msg->addr, msg->mem_addr, mem_addr_type,
                                    msg->buf, msg->len, hal->i2c_state->i2c_conf.timeout);
        }
      } else {
        if (hal->i2c_state->hdma.Instance) {
          HAL_DMA_Init(hal->i2c_state->hi2c.hdmatx);
          status = HAL_I2C_Mem_Write_DMA(&hal->i2c_state->hi2c, msg->addr, msg->mem_addr,
                                         mem_addr_type, msg->buf, msg->len);
        } else if (hal->i2c_state->int_enabled) {
          status = HAL_I2C_Mem_Write_IT(&hal->i2c_state->hi2c, msg->addr, msg->mem_addr,
                                        mem_addr_type, msg->buf, msg->len);
        } else {
          status = HAL_I2C_Mem_Write(&hal->i2c_state->hi2c, msg->addr, msg->mem_addr, mem_addr_type,
                                     msg->buf, msg->len, hal->i2c_state->i2c_conf.timeout);
        }
      }
    } else {
      if (msg->flags & RT_I2C_RD) {
        if (hal->i2c_state->hdma.Instance) {
          HAL_DMA_Init(hal->i2c_state->hi2c.hdmarx);
          mpu_dcache_invalidate(msg->buf, msg->len);
          status = HAL_I2C_Master_Receive_DMA(&hal->i2c_state->hi2c, msg->addr, msg->buf, msg->len);
        } else if (hal->i2c_state->int_enabled) {
          status = HAL_I2C_Master_Receive_IT(&hal->i2c_state->hi2c, msg->addr, msg->buf, msg->len);
        } else {
          status = HAL_I2C_Master_Receive(&hal->i2c_state->hi2c, msg->addr, msg->buf, msg->len,
                                          hal->i2c_state->i2c_conf.timeout);
        }
      } else {
        if (hal->i2c_state->hdma.Instance) {
          HAL_DMA_Init(hal->i2c_state->hi2c.hdmatx);
          status =
              HAL_I2C_Master_Transmit_DMA(&hal->i2c_state->hi2c, msg->addr, msg->buf, msg->len);
        } else if (hal->i2c_state->int_enabled) {
          status = HAL_I2C_Master_Transmit_IT(&hal->i2c_state->hi2c, msg->addr, msg->buf, msg->len);
        } else {
          status = HAL_I2C_Master_Transmit(&hal->i2c_state->hi2c, msg->addr, msg->buf, msg->len,
                                           hal->i2c_state->i2c_conf.timeout);
        }
      }
    }
    ...
    while (1) {
      HAL_I2C_StateTypeDef i2c_state = HAL_I2C_GetState(&hal->i2c_state->hi2c);
    ...
    if (HAL_OK != status) goto exit;
    if (hal->i2c_state->hi2c.ErrorCode) goto exit;

    hal->i2c_state->hi2c.Instance->CR |= I2C_CR_UR;
...
exit:
  ...
  if (status != HAL_BUSY) __HAL_I2C_DISABLE(&hal->i2c_state->hi2c);
#ifdef I2C_HAL_DEBUG
  PBL_LOG(LOG_LEVEL_DEBUG, "master_xfer");
#endif
...
void i2c_hal_abort_transfer(I2CBus *bus) {
  ...
  HAL_I2C_Reset(&(hal->i2c_state->hi2c));
  PBL_LOG(LOG_LEVEL_DEBUG, "reset and send 9 clks");
  __HAL_I2C_DISABLE(&(hal->i2c_state->hi2c));
}
...
void i2c_hal_start_transfer(I2CBus *bus) {
  ...
  if (status == HAL_BUSY) {
#ifdef I2C_HAL_DEBUG
    PBL_LOG(LOG_LEVEL_DEBUG, "i2c wait irq");
#endif
    return;
  }
  ...
  if (status == HAL_OK) {
    bus->state->transfer_event = I2CTransferEvent_TransferComplete;
#ifdef I2C_HAL_DEBUG
    PBL_LOG(LOG_LEVEL_DEBUG, "i2c transfer complete");
#endif
    if (((hal->i2c_state->hdma.Instance == NULL)) && (hal->i2c_state->int_enabled == false))
...
int i2c_hal_configure(struct I2CBusHal *i2c_hal, struct i2c_configuration *configuration) {
  HAL_StatusTypeDef ret = HAL_OK;
  PBL_ASSERTN(i2c_hal != NULL);
  PBL_ASSERTN(configuration != NULL);

  if (configuration->mode & RT_I2C_ADDR_10BIT) {
    i2c_hal->i2c_state->hi2c.Init.AddressingMode = I2C_ADDRESSINGMODE_10BIT;
    i2c_hal->i2c_state->hi2c.Init.OwnAddress1 = (configuration->addr & 0x7fff);
  } else {
    i2c_hal->i2c_state->hi2c.Init.AddressingMode = I2C_ADDRESSINGMODE_7BIT;
    i2c_hal->i2c_state->hi2c.Init.OwnAddress1 = (configuration->addr & 0x7fff) << 1;
  }

  i2c_hal->i2c_state->hi2c.Init.ClockSpeed = configuration->max_hz;
  i2c_hal->i2c_state->hi2c.Init.GeneralCallMode = I2C_GENERALCALL_DISABLE;
  i2c_hal->i2c_state->hi2c.Mode = HAL_I2C_MODE_MASTER;

  HAL_RCC_EnableModule(i2c_hal->module);

  ret = HAL_I2C_Init(&(i2c_hal->i2c_state->hi2c));

  if (ret != HAL_OK) {
    PBL_LOG(LOG_LEVEL_ERROR, "I2C [%s] bus_configure fail!", i2c_hal->i2c_state->device_name);
    return -1;
  }
#ifdef I2C_HAL_DEBUG
  PBL_LOG(LOG_LEVEL_INFO, "I2C [%s] bus_configure ok!", i2c_hal->i2c_state->device_name);
#endif
  return 0;
}
...
bool i2c_hal_is_busy(I2CBus *bus) {
  bool ret = false;
  struct I2CBusHal *hal = (struct I2CBusHal *)bus->hal;
  if (HAL_I2C_GetState(&(hal->i2c_state->hi2c)) != HAL_I2C_STATE_READY) ret = true;
  return ret;
}
...
static int i2c_hal_hw_init(struct I2CBusHal *i2c_hal) {
  ...
  if (i2c_hal->i2c_state->hdma.Instance != NULL) {
    __HAL_LINKDMA(&(i2c_hal->i2c_state->hi2c), hdmarx, i2c_hal->i2c_state->hdma);
    __HAL_LINKDMA(&(i2c_hal->i2c_state->hi2c), hdmatx, i2c_hal->i2c_state->hdma);
    dma_rtx_config.Instance = i2c_hal->i2c_state->hdma.Instance;
    dma_rtx_config.request = i2c_hal->i2c_state->hdma.Init.Request;
    HAL_I2C_DMA_Init(&(i2c_hal->i2c_state->hi2c), &dma_rtx_config, &dma_rtx_config);
  ...
  ret = i2c_hal_configure(i2c_hal, &(i2c_hal->i2c_state->i2c_conf));
...
void i2c_hal_init(I2CBus *bus) {
  ...
  if (ret < 0) {
    PBL_LOG(LOG_LEVEL_ERROR, "I2C [%s] hw init fail!", bus->hal->i2c_state->device_name);
  } else {
    PBL_LOG(LOG_LEVEL_INFO, "I2C [%s] hw init ok!", bus->hal->i2c_state->device_name);
  }
  return;
}
